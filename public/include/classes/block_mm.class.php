...
class Block_mm extends Base {
  protected $table = 'blocks_mm';
  ...
  public function getAverageAmount($limit=10) {
    $stmt = $this->mysqli->prepare("SELECT IFNULL(AVG(amount), " . $this->config['reward_mm'] . ") as avg_amount FROM ( SELECT amount FROM $this->table ORDER BY id DESC LIMIT ?) AS t1");
    if ($this->checkStmt($stmt) && $stmt->bind_param('i', $limit) && $stmt->execute() && $result = $stmt->get_result()) {
...
$block_mm = new Block_mm();
$block_mm->setDebug($debug);
$block_mm->setMysql($mysqli);
$block_mm->setConfig($config);
$block_mm->setErrorCodes($aErrorCodes);
