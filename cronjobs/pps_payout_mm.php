...
if ($config['payout_system_mm'] != 'pps') {
...
if ( $bitcoin_mm->can_connect() === true ){
  $dDifficulty = $bitcoin_mm->getdifficulty();
...
if ($config['pps']['reward']['type'] == 'blockavg' && $block_mm->getBlockCount() > 0) {
  $pps_reward = round($block_mm->getAvgBlockReward($config['pps']['blockavg']['blockcount']));
  $log->logInfo("\tPPS reward using block average, amount: " . $pps_reward . "\tdifficulty: " . $dDifficulty);
} else {
  if ($config['pps']['reward']['type'] == 'block') {
    if ($aLastBlock = $block_mm->getLast()) {
      $pps_reward = $aLastBlock['amount'];
      $log->logInfo("\tPPS value (Last Block): " . $pps_reward);
    } else {
      $pps_reward = $config['pps']['reward']['default_mm'];
...
if (!$iPreviousShareId = $setting->getValue('pps_last_share_id_mm')) {
  $log->logError("Failed to fetch Previous Share ID. ERROR: " . $setting->getCronError());
}
$log->logInfo("\tPPS Last Share ID: " . $iPreviousShareId); 

if (!$iLastShareId = $share_mm->getLastInsertedShareId()) {
  $log->logError("Failed to fetch Last Inserted PPS Share ID. ERROR: " . $share_mm->getCronError());
}
...
if (!$aAccountShares = $share_mm->getSharesForAccounts($iPreviousShareId + 1, $iLastShareId)) {
  $log->logError("Failed to fetch Account Shares. ERROR: " . $share_mm->getCronError());
}
...
foreach ($aAccountShares as $aData) {
  ...
  if ($config['fees_mm'] > 0 && $aData['no_fees'] == 0)
    $aData['fee'] = round($config['fees_mm'] / 100 * $aData['payout'], 8);
  ...
  if (!$transaction_mm->addTransaction($aData['id'], $aData['payout'], 'Credit_PPS'))
    $log->logError('Failed to add Credit_PPS transaction in database: ' . $transaction_mm->getCronError());
  // Add new fee debit for this block
  if ($aData['fee'] > 0 && $config['fees'] > 0)
    if (!$transaction_mm->addTransaction($aData['id'], $aData['fee'], 'Fee_PPS'))
      $log->logError('Failed to add Fee_PPS transaction in database: ' . $transaction_mm->getCronError());
  // Add new donation debit
  if ($aData['donation'] > 0)
    if (!$transaction_mm->addTransaction($aData['id'], $aData['donation'], 'Donation_PPS'))
      $log->logError('Failed to add Donation_PPS transaction in database: ' . $transaction_mm->getCronError());
}

// Store our last inserted ID for the next run
$log->logInfo("\tFetching Last Share ID...");
if (!$setting->setValue('pps_last_share_id_mm', $iLastShareId)) {
  $log->logError("Failed to fetch Last Share ID. ERROR: " . $setting->getCronError());
}

// Fetch all unaccounted blocks
$log->logInfo("\tFetching unaccounted blocks.");
if ($aAllBlocks = $block_mm->getAllUnaccounted('ASC')) {
  ...
  foreach ($aAllBlocks as $iIndex => $aBlock) {
    ...
    if (!$aAccountShares = $share_mm->getSharesForAccounts(@$iLastBlockShare, $aBlock['share_id'])) {
      $log->logError("Failed to Account Shares. ERROR: " . $share_mm->getCronError());
    }
    ...
    if ($aBlock['share_id'] < $iLastShareId) {
      if (!$share_mm->moveArchive($aBlock['share_id'], $aBlock['id'], @$iLastBlockShare))
        $log->logError("Failed to copy shares to from " . $aBlock['share_id'] . " to " . $iLastBlockShare . ' Error: ' . $share_mm->getCronError());
    }
    $log->logInfo("\tBlock: " . $aBlock['id'] . "\t Shares moved to archive...");

    // Delete shares
    $log->logInfo("\tBlock: " . $aBlock['id'] . "\t Deleting accounted shares...");
    if ($aBlock['share_id'] < $iLastShareId && !$share_mm->deleteAccountedShares($aBlock['share_id'], $iLastBlockShare)) {
      $log->logFatal("Failed to delete accounted shares from " . $aBlock['share_id'] . " to " . $iLastBlockShare . ", aborting! Error: " . $share_mm->getCronError());
    ...
    if (!$block_mm->setAccounted($aBlock['id'])) {
      $log->logFatal("Failed to mark block as accounted! Aborting! Error: " . $block_mm->getCronError());
  ...
  $log->logInfo("Failed to fetch unaccounted Blocks. NOTICE: " . $block_mm->getCronError());
}
